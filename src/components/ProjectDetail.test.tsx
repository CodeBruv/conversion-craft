...
describe("ProjectDetail", () => {
  it("renders the case study copy", () => {
    ...
    expect(
      screen.getByRole("heading", { level: 1, name: "Elbi Homes" }),
    ).toBeInTheDocument();
  ...
  it("omits a copy block that has no content instead of showing an empty heading", () => {
    ...
    expect(
      screen.queryByRole("heading", { name: "Context" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("heading", { name: "Outcome" }),
    ).not.toBeInTheDocument();
    expect(
  ...
  it("shows the live project button when there is a live URL", () => {
    ...
    expect(
      screen.getByRole("link", { name: /view live project/i }),
    ).toHaveAttribute("href", "https://elbi-homes.netlify.app/");
  });
  ...
  it("hides the live project button when there is no live URL", () => {
    ...
    expect(
      screen.queryByRole("link", { name: /view live project/i }),
    ).not.toBeInTheDocument();
  });
  ...
  it("keeps the email enquiry wired to the project title", () => {
    renderDetail();

    const link = screen.getByRole("link", {
      name: /i want something like this/i,
    });
    const href = link.getAttribute("href") ?? "";

    expect(href.startsWith("mailto:abdooolmajeeed@gmail.com?")).toBe(true);
    expect(decodeURIComponent(href.split("?")[1] ?? "")).toBe(
      "subject=I want something like this&body=Hi, I saw your Elbi Homes project.%0A%0AI want something similar.",
    );
  });
  ...
  it("renders gallery images when a project has them", () => {
    ...
    expect(
      screen.getByAltText("Elbi Homes - screenshot 1"),
    ).toBeInTheDocument();
    expect(
      screen.getByAltText("Elbi Homes - screenshot 2"),
    ).toBeInTheDocument();
  });
  ...
  it("warns that a draft is not on the live site", () => {
    ...
    expect(
      screen.getByText(/does not exist on the live site/i),
    ).toBeInTheDocument();
  });
  ...
  it("shows no banner for a published project", () => {
    ...
    expect(
      screen.queryByText(/does not exist on the live site/i),
    ).not.toBeInTheDocument();
  });
});
